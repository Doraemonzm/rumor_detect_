...
def parse_option():
    ...
    parser.add_argument("-img_path", type=str,
                        default='data/log/xlnet-base-cased/politi/img_best.pth',
                        metavar='PATH')
    parser.add_argument("-text_path", type=str,
                        default='data/log/xlnet-base-cased/politi/text_best.pth',
                        metavar='PATH')
...
def main():
    ...
    if args.text_path:
        ...
        pretrained = torch.load(args.text_path)['model']
        model_dict = model_text.state_dict()
        pretrain_dict = {
            k: v
        ...
        model_text.load_state_dict(model_dict)

        # model_text.load_state_dict(checkpoint['model'])

    # fc = LinearEmbed(5888, 2)  # 2048+ 768

    acc, prec_0, rec_0, f_0, prec_1, rec_1, f_1 = validate_2_stage(val_loader, model_text)
...
def validate_2_stage(val_loader, model_text):
    """validation"""
    ...
    with torch.no_grad():
        ...
        for idx, data in enumerate(val_loader):
            ...
            acc = accuracy(logits_text, batch_y)
            count_0, count_correct_0, count_target_0 = metric(logits_text, batch_y, for_fake=True)
            count_1, count_correct_1, count_target_1 = metric(logits_text, batch_y, for_fake=False)
